...
const BeerDetailScreen = ({ route }) => {
  ...
  const fetchBeerDetails = () => {
    axios.get(`http://192.168.0.207:3001/api/v1/beers/${id}`)
  ...
  const fetchReviews = async () => {
    ...
    axios.get(`http://192.168.0.207:3001/api/v1/beers/${id}/reviews`)
      .then(response => {
        dispatch({
  ...
  const handleReviewSubmit = async (values) => {
    ...
    if (userId) {
      try {
        const response = await axios.post(`http://192.168.0.207:3001/api/v1/beers/${beer.id}/reviews`, {
          review,
          user_id: userId
        });
